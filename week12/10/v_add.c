...
#pragma omp parallel
    {
        ...
        for (int i = k; i < ARRAY_SIZE; i += gap)
            z[i] = x[i] + y[i];
    }
}
...
#pragma omp parallel
    {
        int gap = omp_get_num_threads();
        int k = omp_get_thread_num();
        int start = ARRAY_SIZE / gap * k;
        int endi;
        if (gap == 1)
            endi = ARRAY_SIZE;
        else if (k != (gap - 1))
            endi = ARRAY_SIZE / gap * (k + 1);
        else
            endi = ARRAY_SIZE;
...
void v_add_autooptimized(double *x, double *y, double *z){
#pragma omp parallel
    {
#pragma omp for
        for(int i=0; i<ARRAY_SIZE; i++)
...
int main() {
    ...
    for (int i = 1; i <= num_threads; i++) {
        ...
        start_time = omp_get_wtime();
        for (int j = 0; j < REPEAT; j++)
            v_add_autooptimized(x, y, z);
        run_time = omp_get_wtime() - start_time;
        if (!verify(x, y, v_add_optimized_chunks)) {
            printf("v_add autooptimized  does not match oracle\n");
            return -1;
        }
        printf("Autooptimized: %d thread(s) took %f seconds\n", i, run_time);
    }
